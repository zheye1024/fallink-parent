package com.xiaozu.core.utils.mark;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.alibaba.fastjson.serializer.SerializerFeature;
import com.xiaozu.core.annotation.MarkTable;
...
public class MarkTableUtil {
     ...
     * @return
     */
    private static String markTable(Object javaBean) {
        String json = null;
        if (javaBean instanceof BaseOrder) {
            if (null != javaBean) {
                ...
                try {
                    ...
                    /* 对克隆实体进行脱敏操作 */
                    MarkTableUtil.replace(ObjectUtil.getAllFields(clone), clone, referenceCounter);
    ...
    public static String markTableJson(Object args) {
        StringBuffer buffer = new StringBuffer();
        if (args instanceof Collection) {
            List<Object> list = (List<Object>) args;
            for (Object obj : list) {
                String message = markTable(obj);
                if (!StringUtils.isBlank(message)) {
                    buffer.append(message);
                }
            }
        } else {
            String message = markTable(args);
            if (!StringUtils.isBlank(message)) {
                buffer.append(message);
            }
        }
    ...
    public static Object markTableObject(Object javaBean) {
        Object object = null;
        try {
            String json = markTable(javaBean);
            if (!StringUtils.isBlank(json)) {
                object = JSONObject.parseObject(json, javaBean.getClass());
            }
        } catch (Throwable e) {
            e.printStackTrace();
        }
    ...
    public static List markTableList(List list) {
        List<Object> resultList = new ArrayList<>();
        if (!CollectionUtils.isEmpty(list)) {
            for (Object object : list) {
                object = markTableObject(object);
    ...
    public static void setNewValueForField(Object javaBean, Field field, Object value) throws IllegalAccessException {
        //处理自身的属性
        MarkTable annotation = field.getAnnotation(MarkTable.class);
        if (field.getType().equals(String.class) && null != annotation && executeIsEffectiveMethod(javaBean, annotation)) {
            String valueStr = (String) value;
            if (StringUtils.isNotBlank(valueStr)) {
                switch (annotation.type()) {
                    case CHINESE_NAME: {
                        field.set(javaBean, MarkTableUtil.chineseName(valueStr));
                        break;
                    }
                    case ID_CARD: {
                        field.set(javaBean, MarkTableUtil.idCardNum(valueStr));
                        break;
                    }
                    case FIXED_PHONE: {
                        field.set(javaBean, MarkTableUtil.fixedPhone(valueStr));
                        break;
                    }
                    case MOBILE_PHONE: {
                        field.set(javaBean, MarkTableUtil.mobilePhone(valueStr));
                        break;
                    }
                    case ADDRESS: {
                        field.set(javaBean, MarkTableUtil.address(valueStr, 8));
                        break;
                    }
                    case EMAIL: {
                        field.set(javaBean, MarkTableUtil.email(valueStr));
                        break;
                    }
                    case BANK_CARD: {
                        field.set(javaBean, MarkTableUtil.bankCard(valueStr));
                        break;
                    }
                    case PASSWORD: {
                        field.set(javaBean, MarkTableUtil.password(valueStr));
                        break;
                    }
                }
            }
     ...
     * @return
     */
    private static boolean executeIsEffectiveMethod(Object javaBean, MarkTable markTable) {
        boolean isAnnotationEffective = true;//注解默认生效
        if (markTable != null) {
            String isEffectiveMethod = markTable.isEffectiveMethod();
